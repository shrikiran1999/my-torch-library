...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        in_channels = []
        in_channels.append(num_input_channels)
        for i in range(self.nlayers-1):
            in_channels.append(num_channels[i])
        self.convolutional_layers = [Conv1d(in_channels[i], num_channels[i], kernel_sizes[i], strides[i],
                                            conv_weight_init_fn, bias_init_fn) for i in range(self.nlayers)]

        self.flatten = Flatten()

        in_width = input_width
        for i in range(self.nlayers):
            self.out = (in_width - kernel_sizes[i])//strides[i] + 1
            in_width = self.out
        self.linear_layer = Linear(self.out*num_channels[-1], num_linear_neurons)
    ...
    def backward(self, labels):
        ...
        # grad = grad.reshape(grad.shape[0], self.num_channels[-1], self.out)
