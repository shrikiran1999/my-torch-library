    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for j in range(self.num_layers):
                h = self.rnn[j].forward(x_current, hidden[j])
                x_current = h
                hidden[j] = h
                # x: (batch_size, seq_len, input_size)
                # hidden -> (num_layers, batch_size, hidden_size)
                # hiddens -> [(seq_len+1) * (num_layers, batch_size, hidden_size)]
            self.hiddens.append(hidden.copy())
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in reversed(range(seq_len)):
            # seq_len-1 to 0
            """ final layer dh already calculated above"""
            for j in reversed(range(self.num_layers)):
                ...
                if j == 0:
                    ...
                    dx, dh[j] = self.rnn[j].backward(dh[j], h, h_prev_l, h_prev_t)
                else:
